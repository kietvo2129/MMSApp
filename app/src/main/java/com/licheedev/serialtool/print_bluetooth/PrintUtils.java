...
public class PrintUtils {
	...
	public static void printText(String string, PrinterInstance mPrinter) {
		mPrinter.init();
		// 换行
	...
	public static void printTextLeft(String string, PrinterInstance mPrinter) {
		mPrinter.init();
	...
	public static void printkhoangtrang(int mint, PrinterInstance mPrinter) {
		mPrinter.init();
		mPrinter.setPrinter(Command.PRINT_AND_WAKE_PAPER_BY_LINE, mint); // 换2行
	}


	public static void printImage(PrinterInstance mPrinter, Bitmap bitmap) {
		mPrinter.init();
		mPrinter.setPrinter(Command.ALIGN, Command.ALIGN_CENTER);
		mPrinter.printImage(bitmap,4);
	}
...
//		cp.setFontProperty(fp);
//		cp.drawText("hasdghadsdasdvasbvftgefgdvfcsfvdgdfdsfvdbnavdghasdvaghdadadavad");
...
//		table.addRow("1," + resources.getString(R.string.coffee) + bitmap
//				+ ";5.00;10.00");
...
//		mPrinter.printBarCode(barcode);
		...
		mPrinter.setPrinter(Command.ALIGN, Command.ALIGN_CENTER);
		mPrinter.setCharacterMultiple(0, 1);
		barcode = new Barcode(BarcodeType.QRCODE, 2, 200, 2, content);
	...
	public static Bitmap convertToBlackWhite(Bitmap bmp) {
		...
		Bitmap resizeBmp = ThumbnailUtils.extractThumbnail(newBmp, 500, 200);
		return resizeBmp;
	}
}
